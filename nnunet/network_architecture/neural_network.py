...
class SegmentationNetwork(NeuralNetwork):
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, mirror_axes: tuple,
        ...
        if all_in_gpu:
            ...
            tumor_cls = np.zeros([1,1], dtype=np.float32)
        ...
        for x in steps[0]:
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    ...
                    predicted_patch, cls_torch = self._internal_maybe_mirror_and_pred_3D(
                        data[None, :, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z], mirror_axes, do_mirroring,
                        gaussian_importance_map)
                    predicted_patch = predicted_patch[0]
                    
                    tumor_cls = cls_torch if cls_torch[0, 0] > tumor_cls[0, 0] else tumor_cls
        ...
        return predicted_segmentation, class_probabilities, tumor_cls
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        result_torch = torch.zeros([1, self.num_classes] + list(x.shape[2:]),
                                   dtype=torch.float)
        cls_torch = torch.zeros([1, 1], dtype=torch.float)

        if torch.cuda.is_available():
            x = to_cuda(x, gpu_id=self.get_device())
            result_torch = result_torch.cuda(self.get_device(), non_blocking=True)
            cls_torch = cls_torch.cuda(self.get_device(), non_blocking=True)
        ...
        for m in range(mirror_idx):
            if m == 0:
                pred, cls_pred = self.inference_apply_nonlin(self(x))
                result_torch += 1 / num_results * pred
                cls_torch += 1 / num_results * cls_pred

            if m == 1 and (2 in mirror_axes):
                pred, cls_pred = self.inference_apply_nonlin(self(torch.flip(x, (4, ))))
                result_torch += 1 / num_results * torch.flip(pred, (4,))

            if m == 2 and (1 in mirror_axes):
                pred, cls_pred = self.inference_apply_nonlin(self(torch.flip(x, (3, ))))
                result_torch += 1 / num_results * torch.flip(pred, (3,))
                cls_torch += 1 / num_results * cls_pred

            if m == 3 and (2 in mirror_axes) and (1 in mirror_axes):
                pred, cls_pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3))))
                result_torch += 1 / num_results * torch.flip(pred, (4, 3))
                cls_torch += 1 / num_results * cls_pred

            if m == 4 and (0 in mirror_axes):
                pred, cls_pred = self.inference_apply_nonlin(self(torch.flip(x, (2, ))))
                result_torch += 1 / num_results * torch.flip(pred, (2,))
                cls_torch += 1 / num_results * cls_pred

            if m == 5 and (0 in mirror_axes) and (2 in mirror_axes):
                pred, cls_pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 2))))
                result_torch += 1 / num_results * torch.flip(pred, (4, 2))
                cls_torch += 1 / num_results * cls_pred

            if m == 6 and (0 in mirror_axes) and (1 in mirror_axes):
                pred, cls_pred = self.inference_apply_nonlin(self(torch.flip(x, (3, 2))))
                result_torch += 1 / num_results * torch.flip(pred, (3, 2))
                cls_torch += 1 / num_results * cls_pred

            if m == 7 and (0 in mirror_axes) and (1 in mirror_axes) and (2 in mirror_axes):
                pred, cls_pred = self.inference_apply_nonlin(self(torch.flip(x, (4, 3, 2))))
