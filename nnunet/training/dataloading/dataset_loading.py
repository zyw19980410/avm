...
class DataLoader3D(SlimDataLoaderBase):
    def __init__(self, data, patch_size, final_patch_size, batch_size, has_prev_stage=False,
        ...
        cls_label_file = 'list.npz'
        with open(cls_label_file, 'rb') as f:
            self.cls_labels = np.load(f)['arr_0']
    ...
    def generate_train_batch(self):
        ...
        for j, i in enumerate(selected_keys):
            ...
            cls_targets.append(self.cls_labels[int(i.split('_')[0])-1])
        ...
        cls_targets=torch.tensor(cls_targets,dtype=torch.float32).reshape((2,1))
