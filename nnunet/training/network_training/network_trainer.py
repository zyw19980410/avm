...
class NetworkTrainer(object):
        ...
        while self.epoch < self.max_num_epochs:
            ...
            if self.use_progress_bar:
                ...
                for _ in range(self.num_batches_per_epoch):
                    np.savez("data1.npz",self.tr_gen.generator.list_of_keys,self.val_gen.generator.list_of_keys)
                    l = self.run_iteration(self.tr_gen, True)
                    train_losses_epoch.append(l)
            
            self.print_to_log_file("clstrainpre:",self.pre)
            self.print_to_log_file("clstrainlab:",self.lab)
            self.trainlab.append(self.lab)
            self.trainpre.append(self.pre)
            ...
            with torch.no_grad():
                ...
                self.print_to_log_file("clsvalpre:",self.pre)
                self.print_to_log_file("clsvallab:",self.lab)
                self.totallab.append(self.lab)
                self.totalpre.append(self.pre)
            ...
            np.savez("data.npz",self.totaldice,self.totallab,self.totalpre,self.trainlab,self.trainpre)
            epoch_end_time = time()
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        cls_target = data_dict['cls_target']
        ...
        if self.fp16:
            with autocast():
                ...
                l += self.cls_loss(cls_output, cls_target)
            ...
            del data
            l, cls_output = self.loss(output, target)
            l += self.cls_loss(cls_output, cls_target)
